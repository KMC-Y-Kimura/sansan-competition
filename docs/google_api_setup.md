...
モック実装(`Mock*`)を実Google APIへ切り替える手順。OAuth client は用途で分ける。同一端末の CLI や同じ端末でのローカル確認なら Desktop app、ローカル LAN で別端末ブラウザから GUI を使う場合もまずは Desktop app を使う。別端末ブラウザ自身で Google 認可まで完了したい場合だけ Web application を使う。
...
4. 認証情報 → OAuthクライアントID を作成する。
   ...
   - ローカル LAN で別端末ブラウザから GUI を使うだけなら **「デスクトップアプリ」**
   - 別端末ブラウザ自身で Google 認可まで完了したいなら **「ウェブ アプリケーション」**
...
- GUI の `/api/live/oauth/*` フローは 2 通りある。ローカル LAN 運用では Desktop app クライアントを使い、認可画面はサーバ端末側の既定ブラウザで開く。別端末ブラウザ自身で Google 認可まで完了したい場合は Web application クライアントが必要。redirect URI の path は `/oauth/google/callback`。
